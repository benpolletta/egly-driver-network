...
from FEF_full3_no_SI_rec_no_VIP import *
...
#        input_beta2_IB=True
    ...
    R8,R9,R10,V_RS,V_FS,V_SI,inp_mon_FEF,R11,R12,R13,R14,mon_RS=all_monitors_FEF
...
#    S_LIP_RS_FEF=generate_syn(RS_sup_LIP,RSvm_FEF,'Isyn_LIP','',0.005*msiemens * cm **-2,0.125*ms,1*ms,0*mV)   
#    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.009*msiemens * cm **-2,0.125*ms,1*ms,0*mV)   
#    S_LIP_RS_FEF=generate_syn(RS_sup_LIP,RSvm_FEF,'Isyn_LIP','',0.018*msiemens * cm **-2,0.125*ms,1*ms,0*mV)   
    S_LIP_RS_FEF=generate_syn(RS_sup_LIP,RSvm_FEF,'Isyn_LIP','',0.009*msiemens * cm **-2,0.125*ms,1*ms,0*mV)  
    
#    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.005*msiemens * cm **-2,0.125*ms,1*ms,0*mV)  
#    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.02*msiemens * cm **-2,0.125*ms,1*ms,0*mV)  
#    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.015*msiemens * cm **-2,0.125*ms,1*ms,0*mV) 
    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.009*msiemens * cm **-2,0.125*ms,1*ms,0*mV)
    
    S_LIP_RSv_FEF=generate_syn(RS_sup_LIP,RSv_FEF,'Isyn_LIP','',0.015*msiemens * cm **-2,0.125*ms,1*ms,0*mV)   
...
#        VIPv_FEF.ginp_VIP2=3* msiemens * cm **-2
...
#    plot(R10.t,R10.i+0+up,'k.',label='VIP')
    plot(R8.t,R8.i+20+up,'r.',label='RS')
    plot(R9.t,R9.i+0+up,'g.',label='SOM')
    xlim(0.2,runtime/second)
    plot([0.2,runtime/second],[up-10,up-10],'k')
#    xticks([],[])
#    legend(loc='upper left') 
...
#    result = adfuller(LFP_LIP)
#    print(f'Test Statistics: {result[0]}')
#    print(f'p-value: {result[1]}')
#    print(f'critical_values: {result[4]}')
#    if result[1] > 0.05:
#        print("LFP_LIP is not stationary")
#    else:
#        print("LFP_LIP is stationary")
#    
#    result = adfuller(LFP_FEF)    
#    print(f'Test Statistics: {result[0]}')
#    print(f'p-value: {result[1]}')
#    print(f'critical_values: {result[4]}')
#    if result[1] > 0.05:
#        print("LFP_FEF_VM is not stationary")
#    else:
#        print("LFP_FEF_VM is stationary")
