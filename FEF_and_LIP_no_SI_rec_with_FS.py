...
from FEF_full3_no_SI_rec_with_FS import *
...
#        input_beta2_IB=True
    ...
    R8,R9,R10,R_FS,V_RS,V_FS,V_SI,R11,R12,R13,R14,mon_RS=all_monitors_FEF
    RSvm_FEF,SIvm_FEF,RSv_FEF,SIv_FEF,VIPv_FEF=all_neurons_FEF[1],all_neurons_FEF[2],all_neurons_FEF[7],all_neurons_FEF[10],all_neurons_FEF[9]
...
#    S_LIP_RS_FEF=generate_syn(RS_sup_LIP,RSvm_FEF,'Isyn_LIP','',0.015*msiemens * cm **-2,0.125*ms,1*ms,0*mV)   
    ...
    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.009*msiemens * cm **-2,0.125*ms,1*ms,0*mV)  
#    S_LIP_FS_FEF=generate_syn(RS_sup_LIP,SIvm_FEF,'Isyn_LIP','',0.018*msiemens * cm **-2,0.125*ms,1*ms,0*mV)  
...
#    
    ...
    plot(R_FS.t,R_FS.i+80,'g.',label='FS')
    xlim(0.2,runtime/second)
    legend(loc='upper left') 
    xlabel('Time (s)')
    ylabel('Neuron index')
...
#    title('FEF Visual-Motor Neurons')
    ...
    plot(R_FS.t,R_FS.i+80,'b.',label='FS')
    xlim(0.2,runtime/second)
    plot([0.2,runtime/second],[up-10,up-10],'k')
#    xticks([],[])
#    legend(loc='upper left') 
