...
def create_FEF_full2(N_RS_vis,N_FS_vis,N_RS_mot,N_dSI_vm,N_RS_vm,N_gSI_vm,theta_phase,target_on,runtime,target_time,g_RSvRSm_AMPA,g_RSvRSm_NMDA,g_RSvmRSm_AMPA,g_RSvmRSm_NMDA):
...
#    S_RSvRSm_AMPA=generate_syn(RS_vis,RS_mot,'IsynRS_FEF_V','',0.04*msiemens * cm **-2,0.125*ms,1*ms,0*mV) #AMPA 0.125*ms,1*ms NMDA 12.5*ms,125*ms
    S_RSvRSm_AMPA=generate_syn(RS_vis,RS_mot,'IsynRS_FEF_V','i<10',g_RSvRSm_AMPA,0.125*ms,1*ms,0*mV) #AMPA 0.125*ms,1*ms NMDA 12.5*ms,125*ms
...
#0.06
    S_RSvmRSm_AMPA=generate_syn(RS_vm,RS_mot,'IsynFS_FEF_V','',g_RSvmRSm_AMPA,0.125*ms,1*ms,0*mV)
    S_RSvRSm_NMDA=generate_syn(RS_vis,RS_mot,'IsynSI_FEF_VM','',g_RSvRSm_NMDA,12.5*ms,125*ms,0*mV) #AMPA 0.125*ms,1*ms NMDA 12.5*ms,125*ms
#    S_RSvRSm_NMDA=generate_syn(RS_vis,RS_vm,'IsynFS_FEF_V','',0.05*msiemens * cm **-2,12.5*ms,125*ms,0*mV) #AMPA 0.125*ms,1*ms NMDA 12.5*ms,125*ms
    S_RSvmRSm_NMDA=generate_syn(RS_vm,RS_mot,'IsynSI2_FEF_VM','',g_RSvmRSm_NMDA,12.5*ms,125*ms,0*mV)
...
if __name__=='__main__':
    ...
    liste_g_v_AMPA=[0.04*msiemens * cm **-2,0.08*msiemens * cm **-2,0.12*msiemens * cm **-2]
    liste_g_v_NMDA=[0.005*msiemens * cm **-2,0.01*msiemens * cm **-2,0.015*msiemens * cm **-2]
    liste_g_vm_AMPA=[0*msiemens * cm **-2]
    liste_g_vm_NMDA=[0.05*msiemens * cm **-2,0.1*msiemens * cm **-2,0.15*msiemens * cm **-2]
#    liste_g_vm_NMDA=[0.20*msiemens * cm **-2,0.25*msiemens * cm **-2,0.30*msiemens * cm **-2]
#    liste_g_vm_NMDA=[0*msiemens * cm **-2]
    
    all_sim=list(product(liste_g_v_AMPA,liste_g_v_NMDA,liste_g_vm_AMPA,liste_g_vm_NMDA))
    ...
    target_on=True
    runtime=1*second
    target_time=550*msecond
    
    path="./results_FEF_full_change_g_mixed_"+str(datetime.datetime.now())
    os.mkdir(path)
    ...
    for simu in all_sim:
        g_v_AMPA,g_v_NMDA,g_vm_AMPA,g_vm_NMDA,index=simu
    ...
    #    net,all_monitors=create_network(N_RS_vis,N_FS_vis,N_RS_mot,N_SI_mot,N_dSI_vm,N_RS_vm,N_gSI_vm,theta_phase,target_on,runtime)
        all_neurons,all_synapses,all_monitors=create_FEF_full2(N_RS_vis,N_FS_vis,N_RS_mot,N_dSI_vm,N_RS_vm,N_gSI_vm,theta_phase,target_on,runtime,target_time,g_v_AMPA,g_v_NMDA,g_vm_AMPA,g_vm_NMDA)
