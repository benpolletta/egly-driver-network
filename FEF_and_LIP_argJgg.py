...
#        input_beta2_IB=True
    ...
    all_neurons_FEF,all_synapses_FEF,all_monitors_FEF=create_FEF_full2(JSOM,gFS,gVS,theta_phase,target_on,runtime,target_time)
...
#    defaultclock.dt = 0.01*ms
    ...
    plot(RSv.t,RSv.i+0,'r.',label='RS')
    plot(FSv.t,FSv.i+20,'b.',label='FS')
    plot(VIPv.t,VIPv.i+40,'k.',label='VIP')
    plot(SIv.t,SIv.i+60,'g.',label='SOM')
    xlim(0.2,runtime/second)
    legend(loc='upper left')   
    xlabel('Time (s)')
    ylabel('Neuron index')
    ...
    savefig(sim_dir+'/FEF_raster.eps')
...
#    title('FEF Visual Neurons')
    plot(RSv.t,RSv.i+0+up,'r.',label='RS')
    plot(FSv.t,FSv.i+20+up,'b.',label='FS')
    plot(VIPv.t,VIPv.i+40+up,'k.',label='VIP')
    plot(SIv.t,SIv.i+60+up,'g.',label='SOM')
    xlim(0.2,runtime/second)
    plot([0.2,runtime/second],[up-10,up-10],'k')
#    xticks([],[])
#    legend(loc='upper left')   
...
#    legend(loc='upper left') 
    ...
    savefig(sim_dir+'/LIP_FEF_raster.eps')
...
#    ylabel('Neuron index')
    ...
    savefig(sim_dir+'/LIP_FEFvm_raster.eps')
...
#    subplot(411)
    ...
    savefig(sim_dir+'/LIP_raster.eps')
