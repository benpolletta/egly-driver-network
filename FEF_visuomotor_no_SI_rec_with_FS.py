...
#    SI_deep.Iapp='8 * uA * cmeter ** -2'
    ...
    FS=NeuronGroup(N_RS_gran,eq_FS_FEF,threshold='V>-20*mvolt',refractory=3*ms,method='rk4')
...
#    FS.J='35 * uA * cmeter ** -2' #article=code=35
    ...
    S_EgranFSgran=generate_syn(E_gran,SI_gran,'IsynRS_FEF_VM','',0.3*msiemens * cm **-2,0.125*ms,1*ms,0*mV) #0.6
    ...
    S_FSgranEgran=generate_syn(SI_gran,E_gran,'IsynSI_FEF_VM','',0.3*msiemens * cm **-2,0.25*ms,20*ms,-80*mV) #0.35
...
#    S_FSgranSIdeep=generate_syn(SI_gran,SI_deep,'IsynSI_FEF_VM','',0.3* msiemens * cm **-2,0.25*ms,20*ms,-75*mV) #1
...
#    S_SIdeepFSgran=generate_syn(SI_deep,SI_gran,'IsynSI2_FEF_VM','',1*msiemens * cm **-2,0.25*ms,20*ms,-80*mV)
    ...
    #From RS to FS and from FS to RS
    S_RSFS=generate_syn(E_gran,FS,'IsynRS_FEF_VM','',1*msiemens * cm **-2,0.125*ms,1*ms,0*mV) #0.4
    S_FSRS=generate_syn(FS,E_gran,'IsynSI2_FEF_VM','',1*msiemens * cm **-2,0.25*ms,5*ms,-80*mV) #0.35
    S_FSFS=generate_syn(FS,FS,'IsynSI_FEF_VM','',1*msiemens * cm **-2,0.25*ms,5*ms,-80*mV) #0.35
...
#                bottomup=vstack((bottomup,generate_spike_timing(N_SI_gran,fLIP,t0,end_time=t1)))
    ...
    R_FS=SpikeMonitor(FS,record=True)
    ...
    all_neurons=SI_deep,E_gran,SI_gran,FS,G_topdown3,Poisson_input,Poisson_input2
#    all_synapses=S_EgranEgran,S_EgranFSgran,S_FSgranEgran,S_FSgranFSgran,S_SIdeepFSgran,topdown_in3,bottomup_in,bottomup_in2
    ...
    all_synapses=S_EgranEgran,S_EgranFSgran,S_FSgranEgran,S_SIdeepFSgran,topdown_in3,bottomup_in,bottomup_in2,S_RSFS,S_FSRS,S_FSFS
    all_monitors=R5,R6,R7,R_FS,V_RS,V_FS,V_SI
...
#    taudinp2=50*ms
    ...
    R5,R6,R7,R_FS,V_RS,V_FS,V_SI=all_monitors
...
#    tight_layout()
    ...
    plot(R_FS.t,R_FS.i+60,'b.',label='FS')
    xlim(0,runtime/second)
#    legend(loc='upper left')
    xlabel('Time (s)')
    ylabel('Neuron index')
