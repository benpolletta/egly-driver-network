...
def generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,runtime,f_mdPul):
...
def run_one_simulation(simu,path):
#    close('all')
    simu=f_mdPul,index
    start_scope()    
...
#    f_mdPul=10*Hz
    ...
    all_neurons,all_synapses,all_monitors=generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,runtime,f_mdPul)    
...
#        current_fig.savefig(new_path+'/figure'+str(n)+'.png')
    ...
    save_raster('FEF_vm_SOM',R6.i,R6.t,new_path) 
    save_raster('FEF_vm_VIP',R7.i,R7.t,new_path) 
    
    
    
liste_fmdPul=[7*Hz,10*Hz,13*Hz,16*Hz,19*Hz,22*Hz,25*Hz,28*Hz,31*Hz,34*Hz]      
N=50

liste_simus=[]
for t in liste_fmdPul:
    liste_simus+=[t]*N
    

path="./results_FEF_mdPul_"+str(datetime.datetime.now())
os.mkdir(path)
    
all_sim=[list(liste_simus[i])+[i] for i in range(len(liste_simus))]

print(len(liste_simus))
...
with parallel_backend('multiprocessing'):
    Parallel(n_jobs=num_cores)(delayed(run_one_simulation)(simu,path) for simu in all_sim)
