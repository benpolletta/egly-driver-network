...
#    FS.Iapp='0 * uA * cmeter ** -2' #article=code=35
    ...
    S_RSFS=generate_syn(RS,FS,'IsynRS_FEF_VM','i//10==j//10',1* msiemens * cm **-2,0.125*ms,1*ms,0*mV) 
    S_RSRS=generate_syn(RS,RS,'IsynRS_FEF_VM','i//10==j//10',0.5* msiemens * cm **-2,0.125*ms,1*ms,0*mV) 
    S_FSRS=generate_syn(FS,RS,'IsynSI_FEF_VM','i//10==j//10',1* msiemens * cm **-2,0.25*ms,5*ms,-80*mV) 
    S_FSFS=generate_syn(FS,FS,'IsynSI_FEF_VM','i//10==j//10',0.3* msiemens * cm **-2,0.25*ms,5*ms,-80*mV) 
...
#            fLIP=50*Hz
            ...
            fLIP=50*Hz
            gamma_background=generate_spike_timing(N_FS,fLIP,0*ms,end_time=125*ms)
            tdeb,tfin,ind=125*ms,250*ms,0
            while tdeb<runtime:
                fLIP=50*Hz*ind+13*Hz*(1-ind)
                gamma_background=vstack((gamma_background,generate_spike_timing(N_FS,fLIP,tdeb,end_time=tfin)))
                ind=(ind+1)%2
                tdeb,tfin=tfin,tfin+125*ms
#        print(fLIP)
#        gamma_background=generate_spike_timing(N_FS,fLIP,0*ms,end_time=2100*ms)
...
#        SI.ginp_SI2=4* msiemens * cm **-2
#        VIP.ginp_VIP2=4* msiemens * cm **-2
#        RS.ginp_RS2=4* msiemens * cm **-2
