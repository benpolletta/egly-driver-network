...
def generate_visual_neurons(theta_phase,N_FS,N_RS,runtime,target_on,target_time,J_SI,J_VIP):
...
#    VIP.Iapp='5 * uA * cmeter ** -2' #article=code=35    
    VIP.Iapp=J_VIP #article=code=35  
...
#        VIP.ginp_VIP_good=3.5* msiemens * cm **-2
#        VIP.ginp_VIP_bad=3.5* msiemens * cm **-2
        ...
        if theta_phase=='mixed':
            ...
            t0=0*ms
            t1=125*ms
            gamma_background=generate_spike_timing(N_FS,50*Hz,t0,end_time=t1)
...
#        SI.ginp_SI2=3* msiemens * cm **-2
        VIP.ginp_VIP2=3.5* msiemens * cm **-2
...
def run_one_sim(simu,path):
#    fLIP,N_simu=simu
    J_SI,J_VIP,N_simu=simu
    ...
    N_FS,N_RS=20,20
    all_neurons,all_synapses,all_monitors=generate_visual_neurons(theta_phase,N_FS,N_RS,runtime,target_on,target_time,J_SI,J_VIP)    
...
#    net.add(M)
    ...
    save_raster('FEF_v_RS',R5.i,R5.t,new_path)
    save_raster('FEF_v_FS',R6.i,R6.t,new_path)
    save_raster('FEF_v_VIP',R7.i,R7.t,new_path)
    save_raster('FEF_v_SI',R8.i,R8.t,new_path)
    
    figure()
    plot(R5.t,R5.i+0,'r.',label='RS')
    plot(R6.t,R6.i+20,'k.',label='FS')
    plot(R7.t,R7.i+40,'b.',label='VIP')
...
if __name__=='__main__':
    ...
    if os.name == 'nt':
        ...
        path="./results_FEFvis_"+str(datetime.datetime.now())
    
    os.mkdir(path)
    
    N=50
    liste_J_SI=['30 * uA * cmeter ** -2','35 * uA * cmeter ** -2','40 * uA * cmeter ** -2','45 * uA * cmeter ** -2','50 * uA * cmeter ** -2']
    liste_J_VIP=['-5 * uA * cmeter ** -2','0 * uA * cmeter ** -2','5 * uA * cmeter ** -2','10 * uA * cmeter ** -2','15 * uA * cmeter ** -2']
    liste_simus=list(product(liste_J_SI,liste_J_VIP))
    
#    liste_fLIP=[10*Hz,13*Hz,16*Hz,19*Hz,22*Hz,25*Hz,28*Hz,31*Hz,34*Hz,37*Hz,40*Hz,43*Hz,46*Hz,49*Hz]
# 
#    liste_simus=[]
#    for t in liste_fLIP:
#        liste_simus+=[t]*N
