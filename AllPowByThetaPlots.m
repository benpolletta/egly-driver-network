...
if isempty(frequencies)
    frequencies = {9:100, 9:40}; %{9:60, 9:40};
end

if nargin < 4, cycle_cell = []; end
if isempty(cycle_cell)
    cycle_cell={4, linspace(4,18,92), 6, linspace(6,24,92), 8, linspace(8,30,92)}; %{4, linspace(4,12,52), 6, linspace(6,18,52), 8, linspace(8,24,52)};
end
    ...
    for s = 1:length(suffixes)
        ...
        for c = 1:length(cycle_cell)
            ...
            if exist([pwd, '/', label, '.mat'], 'file')
                ...
                save([pwd, '/', label], 'data', 'time', 'fs', 'freqs', 'no_cycles', 'ws', '-v7.3')

            end

            time = (1:size(ws, 1))/fs;

            no_thetas = floor(max(time)/.25);
            length_theta = .25*fs;
