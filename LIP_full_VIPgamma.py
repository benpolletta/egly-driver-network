...
from cells.SI_LIP_VIPgamma import *
...
from LIP_superficial_layer_VIPgamma import *
from LIP_beta1_VIPgamma import *
...
#    print(syn_cond,J,thal,theta_phase)    
    ...
    all_neurons, all_synapses, all_gap_junctions, all_monitors=create_Mark_Alex_network(kainate,version,target_time,Nf=NN)
    V1,V2,V3,R1,R2,R3,R4, I1,I2,I3,V4,R4,I4s,I4a,I4ad,I4bd=all_monitors
    RS, FS, SI, VIPinput, IB_soma,IB_axon,IB_bd,IB_ad =all_neurons
    
    SI.ginp_SI=30* msiemens * cm **-2
...
#    print(input_mixed,input_beta2_IB,input_beta2_RS,input_beta2_FS_SI,input_thalamus_gran)
    ...
    SI_deep=NeuronGroup(N_SI,eq_SI_LIP_VIPgamma,threshold='V>-20*mvolt',refractory=3*ms,method='rk4',name='SIdeepLIP')
    SI_deep.V = '-100*mvolt+10*rand()*mvolt'
    SI_deep.h = '0+0.05*rand()'
    SI_deep.m = '0+0.05*rand()'
    SI_deep.mAR = '0.02+0.04*rand()'
    SI_deep.ginp_SI = '30*msiemens*cm**-2'
    if version == 'Alex':
        if kainate=='low':
...
#            next_spike=list_time[-1][0]+(1+0.1*rand())/f
    ...
    VIP_timing=generate_spike_timing(N_SI,50*Hz,target_time,target_time+100*ms)
    VIPinput = SpikeGeneratorGroup(N_SI, VIP_timing[:,1], VIP_timing[:,0]*second)
    S_VIPSI = Synapses(VIPinput,SI_deep,on_pre='Vinp=Vhigh')
    S_VIPSI.connect(j='i')
...
#    print(Poisson_input2)
    ...
    g_inputs=[G_topdown2,G_topdown3,G_lateral,G_lateral2,Poisson_input,Poisson_input2,VIPinput]
    g_inputs=[y for y in g_inputs if y]
...
#        input_beta2_IB=True
    ...
    V1,V2,V3,R1,R2,R3,RVIP,I1,I2,I3,V4,R4,I4s,I4a,I4ad,I4bd,R5,R6,R7,V5,V6,V7,inpmon,inpIBmon=all_monitors
