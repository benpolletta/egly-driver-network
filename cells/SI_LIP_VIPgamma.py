...
Iapp=sinp_VIP*ginp_SI*(V-VK_SI) : amp * meter ** -2
    dsinp_VIP/dt=-sinp_VIP/taudinp2 + (1-sinp_VIP)/taurinp2*0.5*(1+tanh(Vinp/10/mV)) : 1
    dVinp/dt=1/tauinp2*(Vlow-Vinp) : volt
    ginp_SI : siemens * meter **-2

Iapp1=sinp*ginp_SI1*(V-VK_SI) : amp * meter ** -2
    dsinp/dt=-sinp/taudinp2 + (1-sinp)/taurinp2*0.5*(1+tanh(Vinp1/10/mV)) : 1
    dVinp1/dt=1/tauinp2*(Vlow-Vinp1) : volt
...
if __name__=='__main__' :
    ...
    SI=NeuronGroup(1,eq_SI_LIP_VIPgamma,threshold='V>-20*mvolt',refractory=3*ms,method='rk4')
