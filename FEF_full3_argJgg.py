...
def create_FEF_full2(J,gFS,gVS,theta_phase,target_on,runtime,target_time):
    
    N_RS_vis,N_FS_vis,N_RS_mot,N_VIP_vm,N_RS_vm,N_FS_vm,N_gSI_vm=[20]*7
    #create each functional group of neurons individually
    all_neurons_vm,all_synapses_vm,all_monitors_vm=generate_VM_wFS(theta_phase,J,gFS,gVS,runtime)
...
#    net,all_monitors=create_network(N_RS_vis,N_FS_vis,N_RS_mot,N_SI_mot,N_VIP_vm,N_RS_vm,N_gSI_vm,theta_phase,target_on,runtime)
    all_neurons,all_synapses,all_monitors=create_FEF_full2(J,gFS,gVS,theta_phase,target_on,runtime,target_time)
...
#    R1,R2,R3,V1,V2,V3,R4,R5,V4,V5,R6,R7,V6,V7,R8,mon_RS=all_monitors
    RSvm,SIvm,VIPvm,FSvm,V_RS,V_SI,V_VIP,RSv,FSv,VIPv,SIv,RSm=all_monitors
