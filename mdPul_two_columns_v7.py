...
#    E_A.J='10 * uA * cmeter ** -2'  #article SI=25, code=1
    E_A.J='-5 * uA * cmeter ** -2'
...
#    E_B.J='10 * uA * cmeter ** -2'  #article SI=25, code=1
    E_B.J='-5 * uA * cmeter ** -2'

    I_A=NeuronGroup(20,eq_SI_FEF,threshold='V>-20*mvolt',refractory=3*ms,method='rk4')
    I_A.V = '-70*mvolt+10*rand()*mvolt'
    I_A.h = '0+0.05*rand()'
    I_A.m = '0+0.05*rand()'
#    I_A.J = '10 * uA * cmeter ** -2'
    I_A.J = '20 * uA * cmeter ** -2'
    
    I_B=NeuronGroup(20,eq_SI_FEF,threshold='V>-20*mvolt',refractory=3*ms,method='rk4')
    I_B.V = '-70*mvolt+10*rand()*mvolt'
    I_B.h = '0+0.05*rand()'
    I_B.m = '0+0.05*rand()'
#    I_B.J = '10 * uA * cmeter ** -2'
    I_B.J = '20 * uA * cmeter ** -2'
...
#    HTC_A.J='15 * uA * cmeter ** -2'
#    HTC_B.J='15 * uA * cmeter ** -2'
    HTC_A.J='6 * uA * cmeter ** -2'
    HTC_B.J='6 * uA * cmeter ** -2'    
...
#    RE_A.V='-40*mvolt+5*mvolt*rand()'
    ...
    E_A.V='0*mvolt+5*mvolt*rand()'
    I_A.V='0*mvolt+5*mvolt*rand()'
    E_B.V='-50*mvolt+5*mvolt*rand()'
    I_B.V='-50*mvolt+5*mvolt*rand()'
    
    g_TC_E=0.2 * msiemens * cm **-2
#    g_E_HTC=0.2 * msiemens * cm **-2
    g_E_HTC=0.04 * msiemens * cm **-2
    ...
    synEA_HTCA=generate_syn(E_A,HTC_A,'Isyn_FEF','',g_E_HTC,0.125*ms,1*ms,0*mV) 
    synEB_HTCB=generate_syn(E_B,HTC_B,'Isyn_FEF','',g_E_HTC,0.125*ms,1*ms,0*mV) 
#
    g_E_I=0.75 * msiemens * cm **-2
    g_E_E=0 * msiemens * cm **-2
    g_I_E=0.75 * msiemens * cm **-2
    g_I_I=0.75 * msiemens * cm **-2
#    g_E_I=1.25 * msiemens * cm **-2
#    g_E_E=0 * msiemens * cm **-2
#    g_I_E=1.25 * msiemens * cm **-2
#    g_I_I=0.75 * msiemens * cm **-2
    synEA_IA=generate_syn(E_A,I_A,'IsynRS_FEF_V','',g_E_I,0.125*ms,1*ms,0*mV)
    synEA_EA=generate_syn(E_A,E_A,'IsynRS_FEF_V','',g_E_E,0.125*ms,1*ms,0*mV)
    synEB_IB=generate_syn(E_B,I_B,'IsynRS_FEF_V','',g_E_I,0.125*ms,1*ms,0*mV)
    synEB_EB=generate_syn(E_B,E_B,'IsynRS_FEF_V','',g_E_E,0.125*ms,1*ms,0*mV)
    synIA_EB=generate_syn(I_B,E_A,'IsynFS_FEF_V','',g_I_E,0.25*ms,20*ms,-80*mV)
    synIB_EA=generate_syn(I_A,E_B,'IsynFS_FEF_V','',g_I_E,0.25*ms,20*ms,-80*mV)
    synIA_IA=generate_syn(I_A,I_A,'IsynFS_FEF_V','',g_I_I,0.25*ms,20*ms,-80*mV)
    synIB_IB=generate_syn(I_B,I_B,'IsynFS_FEF_V','',g_I_I,0.25*ms,20*ms,-80*mV)
    
    gGABA_A_RE_RE = 0.07 * msiemens * cm **-2
    gGABA_A_RE_HTC = 0.007 * msiemens * cm **-2
...
#    gAMPA_HTC_RE = 0.0225 * msiemens * cm **-2
    ...
    gGABA_A_RE_TC = 0.01 * msiemens * cm **-2
    gGABA_B_RE_TC = 0.02 * msiemens * cm **-2
    gAMPA_HTC_RE = 0.005 * msiemens * cm **-2
    gGABA_A_RE_HTC = 0.001 * msiemens * cm **-2
    gGABA_B_RE_HTC = 0.0001 * msiemens * cm **-2
    ...
    IA=SpikeMonitor(I_A,record=True)
    IB=SpikeMonitor(I_B,record=True)
    
    all_neurons=all_neuronsA+all_neuronsB+(RE_A,RE_B,E_A,E_B,I_A,I_B)
    all_synapses=all_synapsesA,all_synapsesB+(synRERE_A,synREHTC_A,synRETC_A,synHTCRE_A,synTCRE_A,synBREHTC_A,synBRETC_A, synRERE_B,synREHTC_B,synRETC_B,synHTCRE_B,synTCRE_B,synBREHTC_B,synBRETC_B,synTCA_EA,synTCB_EB)
    all_synapses=all_synapses+(synEA_IA,synEB_IB,synEA_HTCA,synEB_HTCB,synIA_EB,synIB_EA,synEA_EA,synEB_EB,synIA_IA,synIB_IB)
    all_monitors=all_monitorsA+all_monitorsB+(RA,RB,EA,EB,IA,IB)
...
#    gapp=0.1*mamp * cmeter ** -2 # in HTC cells
    ...
    R1A,R2A,V1A,V2A,I1A,I2A,I3A,R1B,R2B,V1B,V2B,I1B,I2B,I3B,RA,RB,EA,EB,IA,IB=all_monitors
#    print(all_synapses)
...
#            net.add(elem)
    ...
    #remove random inputs to FS and RS cells
    ...
    plot(EA.t,EA.i+200,'g.',label='E object cortex')
    plot(IA.t,IA.i+220,'k.',label='I object cortex')
    ...
    plot(EB.t,EB.i+450,'g.')
    plot(IB.t,IB.i+470,'k.')
