...
def make_full_network(syn_cond,J,thal,theta_phase,fFEF):
...
#        topdown_in.connect(j='i')
        ...
        inputs_topdown3=generate_spike_timing(N_SI,fFEF,0*ms,end_time=3000*ms)
...
#    print(simu,len(simu))
    ...
    syn_cond,J,thal,theta_phase,fFEF,index=simu
...
#        input_beta2_IB=True
    ...
    print('Network setup')
    all_neurons, all_synapses, all_gap_junctions, all_monitors=make_full_network(syn_cond,J,thal,theta_phase,fFEF)
...
if __name__=='__main__':
    ...
    all_theta=['good']
    all_fFEF=[10*Hz,13*Hz,16*Hz,19*Hz,22*Hz,25*Hz,28*Hz,31*Hz,34*Hz,37*Hz,40*Hz,43*Hz,46*Hz,49*Hz]
    ...
    all_sim=list(product(all_syn_cond,all_J,all_thal,all_theta,all_fFEF))
    index_var=[-1]
