...
def make_full_network(syn_cond,J,thal,f_mdPul,theta_phase):
...
#        inputs_mdpul=generate_spike_timing_theta(N_FS,13*Hz,0*ms,end_time=3000*ms)
        ...
        t0=0*ms
        t1=125*ms
        inputs_mdpul=generate_spike_timing(N_SI,f_mdPul,t0,end_time=t1)
        while t0+250*ms<runtime:
            t0,t1=t0+250*ms,t1+250*ms
            inputs_mdpul=vstack((inputs_mdpul,generate_spike_timing(N_SI,f_mdPul,t0,end_time=t1)))
...
#        input_beta2_IB=True
    ...
    print('Network setup')
    all_neurons, all_synapses, all_gap_junctions, all_monitors=make_full_network(syn_cond,J,thal,f_mdPul,theta_phase)
