...
def generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,f_mdPul,runtime):
...
#    S_SIdeepFSgran=generate_syn(SI_deep,SI_gran,'IsynSI2_FEF_VM','',1*msiemens * cm **-2,0.25*ms,20*ms,-80*mV)
    ...
    if theta_phase=='mixed':
        t0=0*ms
        t1=125*ms
        inputs_topdown3=generate_spike_timing(N_SI,f_mdPul,t0,end_time=t1)
        while t0+250*ms<runtime:
            t0,t1=t0+250*ms,t1+250*ms
            inputs_topdown3=vstack((inputs_topdown3,generate_spike_timing(N_SI,f_mdPul,t0,end_time=t1)))
...
if __name__=='__main__':
    ...
    all_neurons,all_synapses,all_monitors=generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,f_mdPul,runtime)    
