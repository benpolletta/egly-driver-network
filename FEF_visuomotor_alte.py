...
def generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,runtime,Atheta,Asqrtheta,Aalpha):
...
#    S_SIdeepFSgran=generate_syn(SI_deep,SI_gran,'IsynSI2_FEF_VM','',1*msiemens * cm **-2,0.25*ms,20*ms,-80*mV)
    ...
    if theta_phase=='good':
        ...
        SI_deep.Atheta=Atheta
        SI_deep.Asqrtheta=Asqrtheta
        SI_deep.Aalpha=Aalpha
...
#        print(fLIP)
    ...
    G_topdown3 = SpikeGeneratorGroup(N_SI, array([0]), array([0])*second)
    topdown_in3=Synapses(G_topdown3,SI_deep,on_pre='')
    ...
    V_SI2=StateMonitor(SI_deep,'Iapp2',record=True)
...
#    all_monitors=R5,R6,R7,V_RS,V_FS,V_SI
    all_monitors=R5,R6,R7,V_RS,V_FS,V_SI2
    
    print(SI_deep.Iapp2)
    print(SI_deep.Atheta)
    print(SI_deep.Asqrtheta)
    print(SI_deep.Aalpha)
...
if __name__=='__main__':
    ...
    all_neurons,all_synapses,all_monitors=generate_deepSI_and_gran_layers(theta_phase,N_SI,N_RS_gran,N_SI_gran,runtime,Atheta,Asqrtheta,Aalpha)    
...
#    R5,R6,R7,V_RS,V_FS,V_SI=all_monitors
    R5,R6,R7,V_RS,V_FS,V_SI2=all_monitors
