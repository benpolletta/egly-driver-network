...
#    print(input_mixed,input_beta2_SI_deep,input_beta2_RS,input_beta2_FS_SI,input_thalamus_gran)
...
#    inpmon=StateMonitor(E_gran,'Iinp1',record=True)
    ...
    new_monitors = [R5,R6,R7,V5,V6,V7,inpmon,inpIBmon,FEFinput]
    new_monitors = [y for y in new_monitors if y!=None]
    ...
    all_monitors=all_monitors+tuple(new_monitors)
    return all_neurons, all_synapses, all_gap_junctions, all_monitors


def run_one_simulation(simu,path,index_var,input_path):
#    print(simu,len(simu))
    start_scope()
    close('all')
...
#    f,Spectrum_LFP_V_SId=signal.periodogram(LFP_V_SId, 100000,'flattop', scaling='spectrum')
    ...
    def pctMean(mat, ax):
        ...
        if ax == 0:
            matMean = matmul(matMean, diagMean)
        else:
            matMean = matmul(diagMean, matMean)
    ...
    widths = linspace(4, 16, 100)#*fs/(2*freq*pi)
...
#    legend(loc='upper left')
    ...
    def write_variable(monitor, variable, filename):
        t_file=open(filename+'_time.txt','w')
        for elem in monitor.t:
            t_file.write(str(elem)+',')
        t_file.close()

        V_file=open(filename+'_'+variable+'.txt','w')
        for cell in monitor:
            for elem in getattr(cell, variable):
                V_file.write(str(elem)+',')
        V_file.close()

    write_variable(V1, 'V', new_path+'/RS_V')
...
#    all_thal=[0* msiemens * cm **-2]
    ...
    for simu in all_sim:
        run_one_simulation(simu,path,index_var,input_path)
